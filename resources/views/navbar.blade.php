<nav class="navbar navbar-expand-lg navbar-dark bg-dark"> <!-- id="header-menu" -->
	<div class="collapse navbar-collapse" id="navbarSupportedContent">
			...
			<li class="nav-item dropdown">
				...
				<div class="dropdown-menu" aria-labelledby="navbarDropdown">
					@foreach($kelasKet as $arai)
						<a class="dropdown-item @if(!empty($infoKelas)){{ $arai->kelas == $infoKelas ? 'active' : '' }}@endif" href="{{ url('/home/'.$arai->kelas) }}">{{ $arai->kelas }}</a>
					@endforeach
				</div>
			</li>
			<li class="nav-item">
		...
		<form class="form-inline my-2 my-lg-0" method="post" action="{{ url('/process/teacher/search') }}">
			@csrf
			<input class="form-control mr-sm-2" type="text" name="search" @if(!empty($result)) value="{{ $result }}" @endif placeholder="cari guru / matpel.." required>
